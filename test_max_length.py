...
FAISS_TRAIN_SAMPLE_SIZE = 500_000   # Sample size for IVF training to reduce RAM
FAISS_ADD_BATCH_SIZE = 100_000      # Batch size when adding training vectors to FAISS
FAISS_SEARCH_BATCH_SIZE = 50_000    # Batch size when searching test vectors with FAISS
METADATA_LOOKUP_CHUNKSIZE = 1_000_000  # Chunk size for streaming metadata lookup
...
def load_metadata_labels_for_indices(tsv_path: Path, target_indices, label_column: str = 'label',
                                     chunksize: int = METADATA_LOOKUP_CHUNKSIZE) -> dict:
    """
    Stream metadata TSV and return labels only for requested row indices.

    This avoids loading the full metadata file into RAM when only sampled rows
    are needed for cluster labeling.
    """
    ...
    target_indices = np.array(sorted(set(int(idx) for idx in target_indices)), dtype=np.int64)
    ...
    for chunk_df in chunk_iter:
        ...
        local_targets = [idx for idx in target_set if chunk_start <= idx < chunk_end]
        if local_targets:
            labels = chunk_df[label_column].to_numpy()
            for global_idx in local_targets:
                label_lookup[global_idx] = labels[global_idx - chunk_start]
                target_set.remove(global_idx)
...
def analyze_cluster_characteristics(cluster_labels, embeddings=None, 
    ...
    if USE_METADATA_LABELING and metadata_tsv_path and normal_template_path and nonnormal_template_path:
        ...
        if not metadata_tsv_path.exists():
            ...
            requested_indices = []
            unique_clusters_for_sampling = sorted(set(cluster_labels))
            for cluster_id in unique_clusters_for_sampling:
                ...
                cluster_sample_indices[cluster_id] = sample_indices
                requested_indices.extend(sample_indices.tolist())

            metadata_label_lookup = load_metadata_labels_for_indices(
                metadata_tsv_path,
                requested_indices,
                label_column='label',
                chunksize=METADATA_LOOKUP_CHUNKSIZE
            )
            print(f"   ✓ Loaded {len(metadata_label_lookup):,} sampled metadata labels via streaming")
    ...
    if compute_silhouette and embeddings is not None:
        ...
        if n_non_noise > 0:
            ...
            if n_non_noise > silhouette_sample_size:
                sampled_non_noise_pos = np.random.choice(n_non_noise, silhouette_sample_size, replace=False)
                sampled_indices = non_noise_indices[sampled_non_noise_pos]
            else:
                sampled_indices = non_noise_indices
            ...
            if USE_GPU_SILHOUETTE:
                overall_silhouette, silhouette_per_sample_sampled = compute_silhouette_gpu(
                    sampled_embeddings,
                    sampled_labels,
                ...
                if n_non_noise > silhouette_sample_size:
                    print(f"   Dataset large ({n_non_noise:,} samples), subsampling to {len(sampled_indices):,}...")
                silhouette_per_sample_sampled = silhouette_samples(sampled_embeddings, sampled_labels)
                overall_silhouette = silhouette_score(sampled_embeddings, sampled_labels)
                print(f"   ✓ Silhouette Score: {overall_silhouette:.4f}")
            
            silhouette_per_sample = np.full(len(cluster_labels), np.nan)
            silhouette_per_sample[sampled_indices] = silhouette_per_sample_sampled
        else:
    ...
    for cluster_id in tqdm(unique_clusters, desc="Analyzing clusters"):
        ...
        if metadata_label_lookup is not None and normal_events and nonnormal_events:
            ...
            if n_samples < MIN_CLUSTER_SIZE_FOR_LABELING:
                ...
                sample_indices = cluster_sample_indices.get(cluster_id, cluster_indices)
                
                # Count normal vs non-normal from metadata
                for idx in sample_indices:
                    event_label = metadata_label_lookup.get(int(idx))
    ...
    # Summary statistics
    print(f"\n{'='*70}")
    if metadata_label_lookup is not None:
    ...
    if 'label_name' in df.columns and metadata_label_lookup is not None:
...
def fast_cluster_assignment_faiss(training_embeddings, training_cluster_labels, 
    ...
    d = training_embeddings.shape[1]  # Embedding dimension
    
    print(f"      Training data: {len(training_embeddings):,} samples, dim={d}")
    ...
    train_sample_size = min(len(training_embeddings), FAISS_TRAIN_SAMPLE_SIZE)
    if len(training_embeddings) > train_sample_size:
        print(f"         Sampling {train_sample_size:,} points for IVF training...")
        train_sample_idx = np.random.choice(len(training_embeddings), train_sample_size, replace=False)
        train_sample = np.asarray(training_embeddings[train_sample_idx], dtype=np.float32)
    else:
        train_sample = np.asarray(training_embeddings, dtype=np.float32)

    if use_cosine:
        train_sample = normalize(train_sample, norm='l2', copy=False)
    train_sample = np.ascontiguousarray(train_sample, dtype=np.float32)
    index.train(train_sample)
    del train_sample
    gc.collect()
    
    # Add all training data to index
    print(f"      Adding {len(training_embeddings):,} training vectors to index in batches of {FAISS_ADD_BATCH_SIZE:,}...")
    for start in tqdm(range(0, len(training_embeddings), FAISS_ADD_BATCH_SIZE), desc="      Adding", total=(len(training_embeddings) + FAISS_ADD_BATCH_SIZE - 1) // FAISS_ADD_BATCH_SIZE):
        end = min(start + FAISS_ADD_BATCH_SIZE, len(training_embeddings))
        train_batch = np.asarray(training_embeddings[start:end], dtype=np.float32)
        if use_cosine:
            train_batch = normalize(train_batch, norm='l2', copy=False)
        train_batch = np.ascontiguousarray(train_batch, dtype=np.float32)
        index.add(train_batch)
    
    # Set search parameters
    index.nprobe = nprobe  # How many cells to visit during search
    
    print(f"      ✓ Index built! Starting k-NN search...")
    effective_batch_size = batch_size or FAISS_SEARCH_BATCH_SIZE
    print(f"      Processing {len(test_embeddings):,} test samples in batches of {effective_batch_size:,}...")
    
    # Search in batches (memory efficient)
    all_indices = []
    n_batches = (len(test_embeddings) + effective_batch_size - 1) // effective_batch_size
    
    for i in tqdm(range(0, len(test_embeddings), effective_batch_size), desc="      Searching", total=n_batches):
        batch_end = min(i + effective_batch_size, len(test_embeddings))
        batch = np.asarray(test_embeddings[i:batch_end], dtype=np.float32)
        if use_cosine:
    ...
    # Memory cleanup
    del index, quantizer, all_indices, nearest_indices
    gc.collect()
...
def fast_cluster_assignment_sklearn_batched(training_embeddings, training_cluster_labels,
    ...
    if SUBSAMPLE_KNN_TRAINING and len(training_embeddings) > KNN_SUBSAMPLE_SIZE:
        print(f"      Subsampling training set to {KNN_SUBSAMPLE_SIZE:,} rows for sklearn fallback...")
        sample_idx = np.random.choice(len(training_embeddings), KNN_SUBSAMPLE_SIZE, replace=False)
        training_embeddings = np.asarray(training_embeddings[sample_idx], dtype=np.float32)
        training_cluster_labels = training_cluster_labels[sample_idx]

    # Normalize if cosine
    if use_cosine:
        training_norm = normalize(np.asarray(training_embeddings, dtype=np.float32), norm='l2', copy=False)
        test_norm = normalize(np.asarray(test_embeddings, dtype=np.float32), norm='l2', copy=True)
        metric = 'cosine'
    else:
        training_norm = np.asarray(training_embeddings, dtype=np.float32)
        test_norm = np.asarray(test_embeddings, dtype=np.float32)
        metric = 'euclidean'
